...
def test_i_section():
    ...
    section.calculate_geometric_properties()
    perim = (2 * 305) + (4 * 15.4) + 2 * (305 - 9.9 - 2 * 16.5) + (2 * np.pi * 16.5) + 2 * (308 - 2 * 15.4 - 2 * 16.5)
    check.almost_equal(section.get_perimeter(), perim, rel=r_tol)

def test_box_girder_perimeter():
    box_girder = sections.box_girder_section(d=400, b_t=700, b_b=100, t_ft=20, t_fb=20, t_w=12)
...
def test_compound_rectangular_perimeter():
    ...
    rect4 = sections.rectangular_section(d=50, b=50).align_to(rect3, on="bottom").align_to(rect3, on="right", inner=True)
    geom = rect1 + rect2 + rect3 + rect4
    geom.create_mesh([100])
    section = Section(geom)
    section.calculate_geometric_properties()
    assert section.get_perimeter() == (150 + 50 + 50 + 100 + 100 + 50 + 50 + 50 + 50 + 150)
...
def test_compound_rectangular_isection_perimeter1():
    ...
    section = Section(geom)
    section.calculate_geometric_properties()
    perim = b + (4 * tf) + 2 * (b - tw - 2 * r) + (2 * np.pi * r) + 2 * (d - 2 * tf - 2 * r) + (b_p - b) + (2 * t_p) + b_p
    check.almost_equal(section.get_perimeter(), perim, rel=r_tol)
...
def test_compound_rhs_isection_perimeter():
    ...
    section = Section(geom)
    section.calculate_geometric_properties()
    perim = (2 * b_p) + (4 * t_p) + 2 * (b_p - b + 2 * r) + (2 * np.pi * r) + 2 * (d - 2 * r)
    check.almost_equal(section.get_perimeter(), perim, rel=r_tol)
