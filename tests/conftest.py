...
class Helpers:
    ...
    def validate_properties(validation_list, section):
        for entry in validation_list:
            ...
            elif entry['prop'] == 'x_se':
                (val, _) = section.get_sc()
            elif entry['prop'] == 'y_se':
                (_, val) = section.get_sc()
            elif entry['prop'] == 'x_st':
                (val, _) = section.get_sc_t()
            elif entry['prop'] == 'y_st':
                (_, val) = section.get_sc_t()
            elif entry['prop'] == 'As_x':
                (val, _) = section.get_As()
            elif entry['prop'] == 'As_y':
                (_, val) = section.get_As()
            else:
            ...
            msg = (
                f'Property: {entry["prop"]}, '
                f'Calculated: {val:.5e},'
                f'Expected: {entry["val"]:.5e},'
                f'Tolerance: {calc_tol:.5e}'
            )

            assert np.isclose(val, entry['val'], atol=atol, rtol=tol), msg
