...
class Section:
    ...
    Attributes:
        ...
        materials: List of materials in the geometry
        material_groups: List of :class:`~sectionproperties.pre.pre.MaterialGroup` s,
            which contain the finite elements and stress results (if applicable) for
            each defined material
        mesh: Finite element mesh generated by ``triangle``
        num_nodes: Number of nodes in the finite element mesh
        elements: List of finite element objects describing the cross-section mesh
        mesh_search_tree: STRtree to enable efficient lookup of elements in the mesh
        section_props: Class to store calculated section properties
    """
        ...
        with post.plotting_context(title=title, **kwargs) as (fig, ax):
            ...
            if materials and self.materials:
                ...
                ax.tripcolor(
                    ...
                    triangles=self._mesh_elements[:, 0:3],
                    c=c,
                    cmap=cmap,
                )
...
class MaterialGroup:
    ...
    Attributes:
        num_nodes: Number of nodes in the finite element mesh
        material: Material object for the current MaterialGroup
        stress_result: A :class:`~sectionproperties.post.stress_post.StressResult`
            object for saving the stresses of the current material
        elements: A list of finite element objects that are of the current material type
        el_ids: A list of the element IDs of the elements that are of the current
            material type
    """
