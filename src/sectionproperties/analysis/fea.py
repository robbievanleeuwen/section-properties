...
class Tri6:
    ...
    coords : :class:`numpy.ndarray`
        A 2 x 6 array of the coordinates of the tri-6 nodes. The first three columns relate to the
        vertices of the triangle and the last three columns correspond to the mid-nodes.
    node_ids : list[int]
        A list of the global node ids for the current element
    ...
    def geometric_properties(self):
        ...
        -------
        tuple(float)
            Tuple containing the geometric properties and the elastic and shear moduli of the
    ...
    def torsion_properties(self):
        """Calculates the element stiffness matrix for warping analysis and the torsion load vector.
    ...
    def shear_load_vectors(self, ixx, iyy, ixy, nu):
        ...
        nu : float
            Effective Poisson's ratio for the cross-section
        ...
        for gp in gps:
            ...
            f_psi += (
                gp[0]
                * (
                    nu / 2 * np.transpose(np.transpose(B).dot(np.array([[d1], [d2]])))[0]
            ...
            f_phi += (
                gp[0]
                * (
                    nu / 2 * np.transpose(np.transpose(B).dot(np.array([[h1], [h2]])))[0]
    ...
    def shear_warping_integrals(self, ixx, iyy, ixy, omega):
        """Calculates the element shear centre and warping integrals.

        These are required for shear analysis of the cross-section.
        ...
        omega : :class:`numpy.ndarray`
            Values of the warping function at the element nodes

        Returns
        -------
        tuple(float, float, float, float, float, float)
            Shear centre integrals about the x and y-axes *(sc_xint, sc_yint)*, warping integrals
    ...
    def shear_coefficients(self, ixx, iyy, ixy, psi_shear, phi_shear, nu):
        ...
        psi_shear : :class:`numpy.ndarray`
            Values of the psi shear function at the element nodes
        phi_shear : :class:`numpy.ndarray`
            Values of the phi shear function at the element nodes
        nu : float
            Effective Poisson's ratio for the cross-section
    ...
    def monosymmetry_integrals(self, phi):
        ...
        The result is return about the global axes and principal axes.
    ...
    def plastic_properties(self, u, p):
        """Calculate data used for plastic section analysis.
        ...
        tuple(float, float, float, float, bool)
            Element force *(force)*, modulus weighted area properties *(ea, e.qx, e.qy)* and whether
            or not the element is above the line
        """
        # initialise geometric properties
    ...
    def element_stress(
        ...
        Also returns the shape function weights.
        ...
        cx : float
            x position of the elastic centroid
        cy : float
            y position of the elastic centroid
        ixx : float
            Second moment of area about the centroidal x-axis
        iyy : float
            Second moment of area about the centroidal y-axis
        ixy : float
            Second moment of area about the centroidal xy-axis
        i11 : float
            Second moment of area about the principal 11-axis
        i22 : float
            Second moment of area about the principal 22-axis
        phi : float
            Principal bending axis angle
        j : float
        ...
        -------
        tuple(:class:`numpy.ndarray`, :class:`numpy.ndarray`, ...)
            Tuple containing element stresses and integration weights (:math:`\sigma_{zz,n}`,
            :math:`\sigma_{zz,mxx}`, :math:`\sigma_{zz,myy}`, :math:`\sigma_{zz,m11}`,
            :math:`\sigma_{zz,m22}`, :math:`\sigma_{zx,mzz}`, :math:`\sigma_{zy,mzz}`,
            :math:`\sigma_{zx,vx}`, :math:`\sigma_{zy,vx}`, :math:`\sigma_{zx,vy}`,
...
def gauss_points(n):
    """Returns the *n* point Gaussian integration weight and location for a quadratic triangular
    element.

    Parameters
    ----------
    n : int
        Number of Gauss points (1, 3 or 6)

    Returns
    -------
    :class:`numpy.ndarray`
        An *n x 4* matrix consisting of the integration weight and the eta, xi and zeta locations
        for *n* Gauss points
    """
...
def shape_function(coords, gauss_point):
    ...
    ----------
    coords : :class:`numpy.ndarray`
        Global coordinates of the quadratic triangle vertices [2 x 6]
    gauss_point : :class:`numpy.ndarray`
        Gaussian weight and isoparametric location of the Gauss point

    Returns
    -------
    tuple(:class:`numpy.ndarray`, :class:`numpy.ndarray`, float)
        The value of the shape functions *N(i)* at the given Gauss point [1 x 6], the derivative of
        the shape functions in the j-th global direction *B(i,j)* [2 x 6] and the determinant of the
        Jacobian matrix *j*
    """
...
def extrapolate_to_nodes(w):
    ...
    w : :class:`numpy.ndarray`
        Result at the six Gauss points [1 x 6]
...
def principal_coordinate(phi, x, y):
    ...
    ----------
    phi : float
        Principal bending axis angle (degrees)
    x : float
        x coordinate in the global axis
    y : float
        y coordinate in the global axis
...
def global_coordinate(phi, x11, y22):
    ...
    phi : float
        Principal bending axis angle (degrees)
    x11 : float
        11 coordinate in the principal axis
    y22 : float
        22 coordinate in the principal axis
...
def point_above_line(u, px, py, x, y):
    ...
    ----------
    u : :class:`numpy.ndarray`
        Unit vector parallel to the line [1 x 2]
    px : float
    ...
    -------
    bool
        This method returns *True* if the point is above the line or *False* if the point is below
        the line
    """
